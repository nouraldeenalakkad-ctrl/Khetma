...
begin
  ...
  if selected_part.status <> 'available' or exists (select 1 from parts where room_id = target_room_id and part_number < requested_part and status <> 'completed') then raise exception 'part_is_not_available'; end if;
