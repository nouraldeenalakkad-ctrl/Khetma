...
const requireSupabase = () => {
  if (!supabase) throw new Error('Supabase غير مهيأ. أضف متغيرات البيئة أولًا.')
  return supabase
}
...
export const clearSession = (roomId) => localStorage.removeItem(sessionKey(roomId))

const unwrap = ({ data, error }) => {
  if (error) {
    if (error.code === 'PGRST202') {
      throw new Error('دالة Supabase غير محدثة. شغّل schema.sql كاملًا من SQL Editor ثم أعد تحميل الصفحة.')
    }
    ...
    if (error.message?.includes('digest(text')) {
      throw new Error('قاعدة Supabase لم تُحدّث بعد. شغّل ملف supabase/fix-claim-part.sql ثم أعد تحميل الصفحة.')
    }
    throw error
  }
  return data
...
export async function createRoom(name, participantName, password) {
  const result = unwrap(await requireSupabase().rpc('create_room', {
    room_name: name,
...
export async function getRoomState(roomId) {
  const client = requireSupabase()
  const [roomResult, participantsResult, partsResult, messagesResult] = await Promise.all([
    client.from('rooms').select('id,name,creator_name,status,created_at').eq('id', roomId).single(),
    client.from('participants').select('id,room_id,name,current_part,joined_at').eq('room_id', roomId).order('joined_at'),
    client.from('parts').select('id,room_id,part_number,status,participant_id,started_at,completed_at').eq('room_id', roomId).order('part_number'),
    client.from('messages').select('id,room_id,participant_id,message,created_at').eq('room_id', roomId).order('created_at'),
  ])
  const room = unwrap(roomResult)
  ...
  return {
    ...
    parts: parts.map((part) => ({
      ...
      status: part.status,
      reader: participants.find((participant) => participant.id === part.participant_id)?.name || null,
      participantId: part.participant_id,
    })),
    messages: messages.map((message) => ({
      id: message.id,
      participant_id: message.participant_id,
      sender_name: participants.find((participant) => participant.id === message.participant_id)?.name || 'مشارك',
...
export async function claimPart(roomId, partNumber) {
  ...
  if (!session) throw new Error('انتهت جلسة الغرفة.')
  return unwrap(await requireSupabase().rpc('claim_part', {
...
export async function completePart(roomId, partNumber) {
  const session = getSession(roomId)
  if (!session) throw new Error('انتهت جلسة الغرفة.')
  return unwrap(await requireSupabase().rpc('complete_part', {
    target_room_id: roomId,
    target_participant_id: session.participant_id,
    session_token: session.session_token,
    requested_part: partNumber,
...
export async function sendMessage(roomId, participantId, sessionToken, message) {
  return unwrap(await requireSupabase().rpc('send_message', {
    target_room_id: roomId,
...
export async function leaveRoom(roomId) {
  const session = getSession(roomId)
  if (!session) return
  await unwrap(await requireSupabase().rpc('leave_room', {
    target_room_id: roomId,
    target_participant_id: session.participant_id,
    session_token: session.session_token,
  }))
...
export async function deleteRoom(roomId) {
  const session = getSession(roomId)
  if (!session?.owner_token) throw new Error('هذه العملية للمنشئ فقط.')
  await unwrap(await requireSupabase().rpc('delete_room', {
    target_room_id: roomId,
...
export function subscribeToRoom(roomId, onChange) {
  ...
  const channel = client
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'parts', filter: `room_id=eq.${roomId}` }, onChange)
